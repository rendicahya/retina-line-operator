...
def line_worker(img, bool_mask, lines, queue, cpu_count, cpu_id):
    ...
    linestr = np.zeros((slice_height, width, len(lines)), np.int16)
    temp_line_vals = np.array(len(lines), np.float16)
    ...
    for Y in range(y_start, (cpu_id + 1) * slice_height):
        for X in range(width):
            ...
            for angle, line in enumerate(lines):
                ...
                temp_line_vals[angle] = line_sum / line_count

            temp_line_vals.sort()

            linestr[Y - y_start, X] = np.copy(temp_line_vals)
